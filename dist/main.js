...
var LIMINAL_MAX_SCALE = 3;
var LIMINAL_MIN_OPACITY = 0.33;

var getDefaultState = function getDefaultState(root) {
  var bb = root.getBoundingClientRect();
  var minorAxis = Math.min(bb.width, bb.height);
  var thresholdFactor = 0.5; // REVIEW: some sort of smarter calculation here?

  var zoomThreshold = minorAxis * thresholdFactor;
  return {
...
var handlePointerMove = function handlePointerMove(state, els, e) {
  ...
  var pos1 = [e.touches[0].clientX, e.touches[0].clientY];
  var pos2 = [e.touches[1].clientX, e.touches[1].clientY];
...
var handlePointerUp = function handlePointerUp(state, els, e) {
  ...
  if (state.distance > state.zoomThreshold) {
    ...
    var nextLayerIndex = getNextLayerIndex(els.layers, state.activeLayerIndex);
...
var handlePinch = function handlePinch(distance, state, els) {
  var isZoomingIn = distance > state.distance;
  var pinchAmount = distance / state.zoomThreshold; // As user pinches, the top-most layer should grow and fade out
  // but not completely

  var activeLayer = els.layers[state.activeLayerIndex];
  var scale = pinchAmount * LIMINAL_MAX_SCALE;
  var opacity = 1 - pinchAmount * (1 - LIMINAL_MIN_OPACITY);
  activeLayer.style.transform = "scale(".concat(scale, ")");
...
var init = function init() {
  ...
  var state = getDefaultState(root);
  root.addEventListener('pointermove', handlePointerMove.bind(_this, state, els));
  root.addEventListener('pointerup', handlePointerUp.bind(_this, state, els));
};
